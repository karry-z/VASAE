#!/bin/bash

#SBATCH --job-name=012b_tgeo_data
#SBATCH --output=exp/012_p_TgeoMeaning/logs/%x_%j_%a.log
#SBATCH --gpus=1
#SBATCH --time=4:00:00
#SBATCH --array=0-2

cd ~/work/VASAE
VENV_SITE="$(uv run python -c 'import site; print(site.getsitepackages()[0])')"
export LD_LIBRARY_PATH="${VENV_SITE}/nvidia/cusparselt/lib:${VENV_SITE}/nvidia/cusparse/lib:${LD_LIBRARY_PATH}"
nvidia-smi --list-gpus

echo "Running on host $(hostname)"
echo "Started on $(date)"
echo "Directory is $(pwd)"
printf "\n\n"

layers=(2 6 11)
layer="${layers[$SLURM_ARRAY_TASK_ID]}"

SCRATCH="/scratch/b5bq/pu22650.b5bq/VASAE_out/010_soft_align"
SAE_PATH="${SCRATCH}/010_soft_gpt2_L${layer}_k32_a1e-4"
OUTPUT_DIR="exp/012_p_TgeoMeaning/data/L${layer}_k32"

echo "=== 012b: Data-dependent t_geo analysis: layer=${layer} ==="

uv run python scripts/analyze_tgeo_data.py \
    --sae-path "$SAE_PATH" \
    --model-name gpt2 \
    --layer-idx "$layer" \
    --dataset wikitext \
    --dataset-config wikitext-103-raw-v1 \
    --n-samples 500 \
    --batch-size 16 \
    --context-window 5 \
    --output-dir "$OUTPUT_DIR"

echo "done"
