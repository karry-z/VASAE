...
def parse_args():
    ...
    # sae architecture
    p.add_argument("--dim-sparse", type=int, default=0,
                   help="Sparse dim (0 = auto: vocab_size if tied, 8*dim_input otherwise)")
    p.add_argument("--encoder-type", type=str, default="linear", choices=["linear", "mlp"])
    p.add_argument("--sparsity-type", type=str, default="topk",
    ...
    p.add_argument("--tied-decoder", action="store_true",
                   help="Tie decoder to token embeddings (VASAE)")
    p.add_argument("--freeze-decoder", action="store_true")
    p.add_argument("--use-lowrank", action="store_true")
    p.add_argument("--lowrank-coeff", type=float, default=0.1)
    p.add_argument("--use-abs-topk", action="store_true")
    p.add_argument("--anchor-coeff", type=float, default=0.0)
    p.add_argument("--anchor-mode", type=str, default="hard",
    ...
    p.add_argument("--save-dir", type=str,
                   default="/scratch/b5bq/pu22650.b5bq/VASAE_out/online")
...
def main():
    ...
    # --- Lazy imports (nnsight ~10s, transformers ~8s, wandb ~2s) ---
    import datasets
    import transformers
    import wandb
    ...
    logger.info(f"Model: {type(llm).__name__}, dim={dim_input}, vocab={vocab_size}, "
    ...
    train_source = OnlineActivationSource(
        model=nn_model, tokenizer=tokenizer, layer_idx=args.layer_idx,
        text_dataset=train_ds, batch_size=args.train_batchsize,
        max_length=args.max_length,
    )
    eval_source = OnlineActivationSource(
        model=nn_model, tokenizer=tokenizer, layer_idx=args.layer_idx,
        text_dataset=eval_ds, batch_size=args.eval_batchsize,
        max_length=args.max_length,
    )
    test_source = OnlineActivationSource(
        model=nn_model, tokenizer=tokenizer, layer_idx=args.layer_idx,
        text_dataset=test_ds, batch_size=args.eval_batchsize,
        max_length=args.max_length,
    )
    ...
    logger.info(f"SAE: dim_sparse={dim_sparse}, tied={args.tied_decoder}, "
    ...
    eval_metrics = MetricComposer([logitlens_metric, ve_metric, ce_metric])
    ...
    if not args.no_wandb:
        wandb.init(project="VASAE", name=args.exp_name,
    ...
    for epoch in range(args.num_epochs):
        ...
        train_out = trainer.train_epoch(
            train_source, max_batches=args.max_batches,
    ...
    results = {
        ...
        "last_eval": {k: float(v) if isinstance(v, (int, float)) else v
