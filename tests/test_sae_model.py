...
class TestSAEConfig:
    def test_defaults(self):
        ...
        assert cfg.encoder_type == "linear"
        assert cfg.sparsity_type == "none"
...
class TestSAEModelForward:
    ...
    def test_not_return_dict(self):
        model = self._make_model("linear", "topk", 4)
        x = torch.randn(BATCH, DIM_INPUT)
        out = model(x, return_dict=False)
        assert isinstance(out, tuple)
        assert out[0].ndim == 0  # loss scalar
...
class TestTiedDecoder:
    ...
    def test_attach_embedding_wrong_dim(self):
        cfg = SAEConfig(dim_input=DIM_INPUT, dim_sparse=DIM_SPARSE, tied_decoder=True)
        model = SAEModel(cfg)
        emb = nn.Embedding(100, DIM_INPUT)  # vocab != dim_sparse
...
class TestEncodeDecode:
    def test_encode_returns_pre_and_z(self):
        cfg = SAEConfig(dim_input=DIM_INPUT, dim_sparse=DIM_SPARSE, sparsity_type="topk", k=4)
        model = SAEModel(cfg)
        x = torch.randn(BATCH, DIM_INPUT)
    ...
    def test_decode_shape(self):
        cfg = SAEConfig(dim_input=DIM_INPUT, dim_sparse=DIM_SPARSE)
        model = SAEModel(cfg)
        z = torch.randn(BATCH, DIM_SPARSE)
        out = model.decode(z)
        assert out.shape == (BATCH, DIM_INPUT)

    def test_nonneg_latents(self):
        cfg = SAEConfig(
            dim_input=DIM_INPUT, dim_sparse=DIM_SPARSE,
            nonneg_latents=True, sparsity_type="none",
        )
        model = SAEModel(cfg)
        x = torch.randn(BATCH, DIM_INPUT)
        pre, z = model.encode(x)
    ...
    def test_no_nonneg_latents(self):
        cfg = SAEConfig(
            dim_input=DIM_INPUT, dim_sparse=DIM_SPARSE,
            nonneg_latents=False, sparsity_type="none",
        )
        model = SAEModel(cfg)
        torch.manual_seed(0)
        x = torch.randn(BATCH, DIM_INPUT)
