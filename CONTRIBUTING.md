...
VASAE/
├── src/vasae/          # 核心库
│   ├── models/         # SAE 模型（sae.py, decompose_sae.py, dualpath_sae.py 等）
│   ├── data/           # 数据集与数据 schema
│   ├── engine/         # 训练与评估循环
...
- 正式实验：`编号_F_名称`，如 `012_F_TgeoMeaning/`（`_F_` 标记正式实验）
...
1. **`src/vasae/models/`** — 定义新模型类，参考已有的 `sae.py`、`decompose_sae.py`、`dualpath_sae.py`
